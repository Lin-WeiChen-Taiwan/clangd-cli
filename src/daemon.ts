...
import { ensureSessionDirectory, removeSessionArtifacts, sessionPaths, writeMetadata, type SessionPaths } from "./ipc.js";
import { FileLogger } from "./logger.js";
import { CliError, type AgentRequest, type AgentResponse, type Operation, type SessionConfig } from "./protocol.js";
...
export class Daemon {
  ...
  private session: ClangdSession | undefined;
  private sessionStart: Promise<ClangdSession> | undefined;
  private idleTimer?: NodeJS.Timeout;
  private servedQuery = false;
  ...
  private accept(socket: Socket): void {
    ...
    socket.on("data", (chunk: string) => {
      ...
      try {
        request = JSON.parse(buffer.slice(0, newline)) as AgentRequest;
      } catch (error) {
      ...
      void this.handle(request).then((response) => {
        socket.end(`${JSON.stringify(response)}\n`);
        if (request.operation === "stop") setTimeout(() => void this.shutdown(0), 25);
      });
    });
    socket.on("error", (error) => this.logger.warn(`IPC client error: ${error.message}`));
  }

  private async handle(request: AgentRequest): Promise<AgentResponse> {
  ...
  private async executeWithRetry(request: AgentRequest) {
  ...
  private async ensureSession(): Promise<ClangdSession> {
    if (this.session?.alive) return this.session;
  ...
  private successResponse(operation: Operation, result: unknown, reused: boolean, started: number): AgentResponse {
    return {
      version: 1,
      ok: true,
      operation,
  ...
  private errorResponse(operation: Operation, error: CliError, reused: boolean, started: number): AgentResponse {
  ...
  private async acquireLock(): Promise<boolean> {
    try {
      ...
      if (oldPid > 0 && isProcessAlive(oldPid)) return false;
